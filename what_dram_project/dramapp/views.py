...
# select the appropriate template to use
    ...
    comment_list = Comment.objects.filter(is_public=True, is_removed=False).order_by('submit_date').reverse()[:3]
...
def rate(request, whiksy_id):
    snippet = get_object_or_404(Snippet, pk=whisky_id)
    if 'rating' not in request.GET or request.GET['rating'] not in ('1', '-1'):
        return HttpResponseRedirect(whiksy.get_absolute_url())
    try:
        rating = Rating.objects.get(user__pk=request.user.id, snippet__pk=whiksy.id)
